...
class TBRPGCOMBATSYSTEM_API UCharacterHandler : public UActorComponent
{
	GENERATED_BODY()
...
public:	
	...
	UCharacterHandler();
	UCharacterHandler(int level, int hp, int mAttack, int rAttack,
...
public:
	...
	void DealDamage(UCharacterHandler* target, int movePower, char isMeleeAttack,
	...
	void UseMove(UAttackMoves* attackMove, UCharacterHandler* target);
	int TypeCheck(UAttackMoves* move, UCharacterHandler* target);
